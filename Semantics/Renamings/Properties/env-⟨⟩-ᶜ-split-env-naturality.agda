...
env-⟨⟩-ᶜ-split-env-nat {Γ} {[]} {A} zero p =
  begin
       η
    ∘ᵐ idᵐ
  ≡⟨ ∘ᵐ-identityʳ _ ⟩
  ...
  ≡⟨ ∘ᵐ-congˡ (sym ⟨⟩-idᵐ) ⟩
    ...
    ∘ᵐ η
  ∎
env-⟨⟩-ᶜ-split-env-nat {Γ} {Γ' ∷ B} {A} zero p =
  begin
       η
  ...
  ≡⟨ ∘ᵐ-congʳ (∘ᵐ-congˡ (sym ⟨⟩-idᵐ)) ⟩
    ...
    ∘ᵐ η
  ∎
env-⟨⟩-ᶜ-split-env-nat {Γ} {Γ' ∷ B} {A} (suc τ) p =
  begin
       (   env-⟨⟩-ᶜ {Γ'} {⟦ Γ ⟧ᵉᵒ A} (suc τ) p
        ∘ᵐ fstᵐ)
    ∘ᵐ ⟨ split-env {Γ' = Γ} {Γ'' = Γ'} (≡-split refl) ∘ᵐ fstᵐ , idᵐ ∘ᵐ sndᵐ ⟩ᵐ
  ≡⟨ ∘ᵐ-assoc _ _ _ ⟩
       env-⟨⟩-ᶜ {Γ'} {⟦ Γ ⟧ᵉᵒ A} (suc τ) p
  ...
  ≡⟨ sym (∘ᵐ-assoc _ _ _) ⟩
       (   env-⟨⟩-ᶜ {Γ'} {⟦ Γ ⟧ᵉᵒ A} (suc τ) p
        ∘ᵐ split-env {Γ' = Γ} {Γ'' = Γ'} (≡-split refl))
    ∘ᵐ fstᵐ 
  ≡⟨ ∘ᵐ-congˡ (env-⟨⟩-ᶜ-split-env-nat {Γ} {Γ'} {A} (suc τ) p) ⟩
  ...
  ≡⟨ ∘ᵐ-congʳ (∘ᵐ-congʳ (∘ᵐ-congˡ
      (cong (env-⟨⟩-ᶜ {Γ ++ᶜ Γ'} (suc τ)) (≤-irrelevant _ _)))) ⟩
       ⟨ suc τ ⟩ᶠ (split-env {Γ' = Γ} {Γ'' = Γ' -ᶜ suc τ} (≡-split refl))
    ∘ᵐ ⟨ suc τ ⟩ᶠ ⟦ eq-ren (++ᶜ-ᶜ {Γ} {Γ'} {suc τ} p) ⟧ʳ
    ∘ᵐ env-⟨⟩-ᶜ {Γ ++ᶜ Γ'} {A} (suc τ)
         (≤-trans p
          (≤-trans
    ...
    ∘ᵐ fstᵐ
  ∎
env-⟨⟩-ᶜ-split-env-nat {Γ} {Γ' ⟨ τ' ⟩} {A} zero p =
  begin
       η
  ...
  ≡⟨ ∘ᵐ-congʳ (∘ᵐ-congˡ (sym ⟨⟩-idᵐ)) ⟩
    ...
    ∘ᵐ η
  ∎
env-⟨⟩-ᶜ-split-env-nat {Γ} {Γ' ⟨ τ' ⟩} {A} (suc τ) p with suc τ ≤? τ'
... | yes q =
  begin
       (   μ⁻¹
...
... | no ¬q =
  ...
  ≡⟨ ∘ᵐ-congʳ (∘ᵐ-congʳ (∘ᵐ-congʳ (∘ᵐ-congʳ (cong ⟨ τ' ⟩ᶠ
      (cong (env-⟨⟩-ᶜ {Γ ++ᶜ Γ'} (suc τ ∸ τ')) (≤-irrelevant _ _)))))) ⟩
       ⟨ suc τ ⟩ᶠ (split-env {Γ' = Γ} {Γ'' = Γ' -ᶜ (suc τ ∸ τ')} (≡-split refl))
    ∘ᵐ ⟨ suc τ ⟩ᶠ ⟦ eq-ren (++ᶜ-ᶜ (≤-trans (∸-monoˡ-≤ τ' p) (≤-reflexive (m+n∸n≡m (ctx-time Γ') τ')))) ⟧ʳ
    ∘ᵐ ⟨⟩-≤ (m≤n+m∸n (suc τ) τ')
    ∘ᵐ μ
